...
TimescaleDB ist ein relationales Open-Source Datenbankmanagementsystem<!-- koppeln --> für Zeitreihendaten. Das System setzt SQL als Abfragesprache ein und ist damit genauso leicht bedienbar wie herkömmliche relationale Datenbanken. Zusätzlich skaliert es aber auf eine Weise, wie es ansonsten nur bei NoSQL-Datenbanken möglich ist [3.3.1].
...
Jede Zeile stellt hier ein neuer<!-- einen neuen --> Messwert dar, der jeweils einen Satz an Messungen enthält. Mit diesem Modell können Beziehungen innerhalb der Daten aufrechterhalten werden. Zusätzlich unterstützt das Datenmodell von Timescale JOINs, zum Beispiel für zusätzliche Metadaten wie etwa die genauen Standortdaten, die in einer separaten Tabelle gespeichert werden und den genauen Standort für die IDs in der Spalte location_id angibt<!-- angeben -->. Ohne die Verwendung von JOINs müssen die Metadaten zu jeder Messung gespeichert werden, was zu einer größeren Menge an Daten führt, die zudem schwieriger zu verwalten sind. Dadurch ist eine leichtere Aktualisierung der Daten möglich.<!-- Quelle, auch wenn sie doppelt genannt wird -->

### Narrow-Table-Modell
Die meisten TSDBs stellen ihre Daten folgendermaßen dar:
-	Jede Metrik als separater Eintrag
-	Für jede Metrik gibt es eine Folge von time-/value-Paaren <!-- Ich würde das groß schreiben und einfach koppeln: "Time-Value-Paaren" -->
-	Metadaten werden als „Tag-Set“ repräsentiert, der<!-- vllt. eher: das --> mit der Metrik verbunden ist. 

Dadurch stellt jede Kombination aus der Metrik mit dem Tag-Set eine individuelle Zeitreihe dar, die eine Folge von time-/value-Paaren<!-- vgl. oben?: "Time-Value-Paaren" --> beinhaltet. Das Beispiel aus dem Wide-Table-Modell würde dann zu neun<!-- neuen --> Zeitreihen führen, die jeweils durch einzigartige Tag-Sets definiert sind. Die Spalten cpu_1m_avg, free_mem und temperature würden dabei als drei verschiedene Einträge angesehen werden. Die Daten sind nachfolgend aufgezeigt.<!-- Eigennamen vllt. kursiv, mit Sternchen -->
...
Die Streaming-Replikation bei TimescaleDB ist dieselbe, die auch in PostgreSQL als integrierte Replikation zur Verfügung steht, da Timescale<!-- TimescaleDB --> wie bereits erwähnt, auf PostgreSQL aufbaut. Die Replikation stellt sicher, dass Daten, die in eine primäre Datenbank geschrieben werden, auf einem oder mehreren anderen Knoten gespiegelt werden. Fällt nun die primäre Datenbank aus, kann sie durch einen der Replikationsknoten ersetzt werden und es kommt zu keinen Systemausfällen<!-- vllt. besser: "... es kommt nicht zu Systemausfällen aufgrund eingeschränkter Verfügbarkeit-->. Die Knoten, auf denen die Daten gespiegelt werden, können als reine Read Replicas<!-- kursiv (wg. Eigenname)--> definiert sein, also nur den Lesezugriff ermöglichen. Dadurch wird eine horizontale Skalierung der Lesevorgänge ermöglicht, indem das Volumen der Leseabfragen auf mehrere Knoten verteilt werden<!-- wird (Volumen) --> [3.3.6].
...
Zum Einsatz von Replikation bei TimescaleDB sind mindestens zwei separate Instanzen erforderlich. Weiter<!-- vllt besser: Darüber hinaus--> wird eine logische Replikation nicht empfohlen, da die dafür benötigten Mechanismen von Timescale<!-- TimescaleDB --> aktuell nicht unterstützt werden [3.3.6].

## Unterstützte Programmiersprachen

TimescaleDB wurde in der Programmiersprach C implementiert. Darüber hinaus werden die weiteren Programmiersprachen .Net, C++, Delphi, Java info, JavaScript, Perl, PHP, Python, R, Ruby, Scheme und Tcl unterstützt. [3.3.4]

Als Abfragesprache steht in TimescaleDB SQL vollumfänglich zur Verfügung, was unter anderem JOINS miteinschließt<!-- zwei Wörter--> [3.3.2].
...
-	Dashboards,
-	QA-<!-- vllt. ausschreiben und/oder deutschen Begriff verwenden --> und Leistungstests.
...
#### PostgreSQL
Dadurch, dass Timescale auf PostgreSQL basiert, setzt es auf ein<!-- löschen --> SQL ohne Einschränkungen. Außerdem sind in Timescale<!-- TimescaleDB --> auch PostreSQL-Erweiterungen und Tools verfügbar [3.3.8].
...
Im Vergleich zu anderen TSDBs bietet Timescale<!-- TimescaleDB --> also vor allem in der höheren Geschwindigkeit und der geringeren notwendigen Speicherkapazität Vorteile. Gegenüber anderen Systemen ist Timescale<!-- TimescaleDB --> somit allerdings eher nicht für die Speicherung geringer Datenmengen geeignet, also zum Beispiel eine Speicherung von Daten ohne Metadaten. Dafür sind andere Datenbanken wie beispielsweise InfluxDB wiederum besser geeignet [3.3.10].

Timescale hat bei geringen Kardinalitäten wie zum Beispiel<!-- Doppelung, (2x zum Beispiel) --> bei der Menge von 100 Geräten eine schlechtere Leistung als zum Beispiel InfluxDB [3.3.11].
...
<!-- hinter jeden Absatz, der nicht Deinem Wissen "entspringt", muss eine Quellenangabe, nach dem Absatz und außerhalb eines Satzes. Auch wenn sie sich wiederholt. ;) -->

...
Für Sprachen wie Go, Java, PHP und Python verfügt InfluxDB zudem über leistungsstarke Client-Bibliotheken.
...
Es kann als relationale Datenbank über die CSV-Syntax und als nicht relationale Datenbank in Zuge der TSI und TSM eingesetzt werden. Zudem können durch die Schemafreiheit auch schwächer strukturierte Daten ohne einheitliche Struktur abgespeichert werden. 
...
Grundsätzlich ist ein Nachteil beim Sharding, dass es umständlich sein kann, bei bereits festgelegten Untergruppen weitere Server hinzuzufügen, da dies sehr umfangreiches Umspeichern auf den Servern erfordert.   
Dadurch, dass die Untergruppen bzw. TSM in InfluxDB durch ihre Zeitfenster definiert werden, stellt das Hinzufügen weiterer Untergruppen keine Schwierigkeit dar. Das Zeitfenster wächst mit der Zeit logisch weiter und es ist keine weitere Eingrenzung der Untergruppen erforderlich. 
Allerdings können die flexiblen Aufbewahrungsrichtlinien (siehe Partitionierungsmechanismus) Hindernisse mit sich bringen. An dieser Stelle ist anzumerken, dass auch durch die Schemafreiheit auf die Prüfung der Daten im DBMS verzichtet wird [3.1.7].
Da die Abfrage primär über bestimmte Zeitintervalle erfolgt, stellt auch die Schwierigkeit der Abfrage über Joins beim Sharding oder andere Kriterien als das Aufteilungskriterium kein Problem dar. Durch die Abfrage der Zeitintervalle werden die Untergruppen direkt adressiert.
Grundsätzlich werden in NoSQL-Systemen Joins kaum unterstützt. Deswegen kann das Sharding für NoSQL-Systeme wie InfluxDB ohne weitere Probleme implementiert werden [3.1.5].  
Durch die starke Spezialisierung als TSDB ist InfluxDB auf die Zeitindexierung angewiesen. Eine gute Performance der Abfragen und Verarbeitungen kann somit nur gewährleistet werden, wenn die zeitliche Abfolge stimmt. Durch ihren Aufbau ist sie nur bedingt für das Speichern von anderen, nicht-zeitbezogenen Werten geeignet.
Durch die Verwendung einer internen Abfragesprache muss sich erst an eine neue Sprache gewöhnt werden. Auch wenn diese große Ähnlichkeit mit SQL aufweist, gibt es Unterschiede, die die Einführung im Vergleich zur Verwendung von SQL erschweren.
Zuletzt sollte der Einsatz als Open-Source-Single-Server-Version und die Konzeption für die Anwendung in Docker und der Cloud angemerkt werden [3.1.9]. 
Dadurch, dass InfluxDB auch über eine Enterprise-Version verfügt, ist die Open-Source-Version in gewissen Bereichen eingeschränkt.

### Besonderheiten

InfluxDB verfügt durch das Shading im Vergleich zu anderen TSDB über eine sehr hohe Skalierbarkeit und ist somit unsensibel was die Verarbeitung von Big Data angeht. Hinzukommende Zeiträume werden partioniert und die Performance der Datenbank wird durch ein ansteigendes Datengehalt nicht negativ beeinflusst.
Auch die erwähnte Flexibilität der Speicherung und Verarbeitung von Daten lässt InfluxDB gegenüber anderen TSDBs hervorstechen und lässt Freiraum für die Entwicklung innerhalb der Datenbank.

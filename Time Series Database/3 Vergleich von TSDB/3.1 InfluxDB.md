...
Für die *timestamps* (Zeitstempel) werden als Spalte *_time* die Daten mit einem Zeitstempel indexiert. Die Auflösung der Zeitstempel ist variabel und kann bis zu einer Nanosekunde dauern. Die Daten werden in *field keys* und *field values* gespeichert. Aus der Zusammenführung aller *field keys* und *field values* setzen sich die *field sets* zusammen. Diese stehen immer in Verbindung zu einem *timestamp*. Die *tags* bestehen aus *tag keys* und *tag values*, die als Strings und Metadaten gespeichert werden. 

Die Struktur der hier benannten Elemente zeigt folgende Tabelle:

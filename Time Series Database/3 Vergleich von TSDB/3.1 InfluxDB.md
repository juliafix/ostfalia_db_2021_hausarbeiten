...
InfluxDB wurde im Jahr 2013 entwickelt und ist mittlerweile eine<!--eines--><!-- Grit: "eine" stimmt, da DB feminin -->der am weitesten verbreitete<!--verbreiteten--> TSDB. InfluxDB ist eine NOSQL-TSDB<!-- einheitliche Schreibweise, NoSQL wie unten--> und eine Open-Source-Datenbank.
...
Während *tag keys* optional sind, wird<!--werden--> für die Datenstruktur von InfluxDB *timestamps* und *field keys* voraussgesetzt. Allerdings ist der Vorteil von *tags*, dass diese im Gegensatz zu *fields* indexiert sind und Abfragen deswegen schneller durchführen können und besser für die Speicherung häufig abgefragter Metadaten sind. Bei der Abfrage von *fields* wird jeder einzelne *field value* geprüft. Bei der Abfrage von *keys* ist dies nicht nötig und die Abfrage läuft daher schneller.
...
Zur Anzeige von Rohdaten im Daten-Explorer und zur Rückgabe von Abfrageergebnisse <!--Abfrageergebnissen-->kann zudem ein tabellarisches Datenschema in kommentierter CSV-Syntax verwendet werden [3.1.3].   
...
## Einsatzbereiche
Die Anwendung von InfluxDB ist sinnvoll in Bereichen, in denen Zeitreihendaten generiert werden, die sehr komplex sind und die Kapazitäten herkömmlicher Datenbanken übersteigen (zum Beispiel auch durch das Vorliegen von halbstrukturierten<!--"von" löschen und "halbstrukturierter" statt halbstrukturierten schreiben--> Daten).
...
Es kann als relationale Datenbank über die CSV-Syntax und als nicht relationale Datenbank in <!--im Zuge-->Zuge der TSI und TSM eingesetzt werden. Zudem können durch die Schemafreiheit auch schwächer strukturierte Daten ohne einheitliche Struktur abgespeichert werden.
...
Die flexiblen Aufbewahrungsrichtlinien (siehe Partitionierungsmechanismus) Hindernisse mit sich bringen<!--... bringen Hindernisse mit sich.-->. Durch das hohe Maß an Flexibilität ist die Effizienz der Datenverarbeitung nicht gewährleistet, wenn zum Beispiel die Aufbewahrungsrichtlinien der Shards nicht sinngemäß sind. Je nach Zweck der Datenverarbeitung sollte daher ein hohes Maß an Fachkentnissen für die Entwicklung vorausgesetzt werden um eine hohe Performance gewährleisten zu können.
...
Durch die Verwendung einer internen Abfragesprache muss sich erst an eine neue Sprache gewöhnt werden. Auch wenn diese große Ähnlichkeit mit SQL aufweist, gibt es Unterschiede, die<!--welche--> die Einführung im Vergleich zur Verwendung von SQL erschweren.
...
Auch die erwähnte Flexibilität der Speicherung und <!--die-->Verarbeitung von Daten lässt InfluxDB gegenüber anderen TSDBs hervorstechen und lässt Freiraum für die Entwicklung innerhalb der Datenbank. <!--Quelle?-->
